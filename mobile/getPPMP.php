...
$queryResult=$connect->query("
	SELECT projects.title, projects.created_at, departments.name departmentname
	from projects
...
$queryResult2=$connect->query("
	...
	LEFT OUTER JOIN departments
	on department_budgets.id = departments.id
	where projects.user_id = '".$UserID."' AND projects.is_approved = '1'
	");
...
$queryResult3=$connect->query("
	...
	LEFT OUTER JOIN departments
	on department_budgets.id = departments.id
	where projects.user_id = '".$UserID."' AND projects.is_approved = '0'
	");
...
$queryResult4=$connect->query("
	...
	LEFT OUTER JOIN departments
	on department_budgets.id = departments.id
	where projects.user_id = '".$UserID."' AND projects.is_approved is NULL
	");
